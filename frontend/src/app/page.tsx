...
interface LeadList { id: string; list_name: string; filename: string|null; row_count: number; status: string; color: string; imported_at: string|null; }
...
export default function Home(){
  ...
  const[theme,setTheme]=useState<'dark'|'light'>(()=>typeof window!=='undefined'&&localStorage.getItem('rgs_theme')==='light'?'light':'dark');
  const[lists,setLists]=useState<LeadList[]>([]);const[activeList,setActiveList]=useState<string|null>(null);const[groupByCompany,setGroupByCompany]=useState(false);
  ...
  const load=()=>{api('/stats').then(r=>r.json()).then(setStats).catch(()=>{});fetchLeads();api('/lists').then(r=>r.json()).then(setLists).catch(()=>{});};
  const fetchLeads=(t?:string|null,append?:boolean,batchId?:string|null,grouped?:boolean)=>{let url=`/leads?limit=500&offset=${append?leads.length:0}`;if(t==='scout-lab-source'){url+=`&source=scout-lab`;}else if(t){url+=`&tier=${t}`;}const bid=batchId!==undefined?batchId:activeList;if(bid)url+=`&batch_id=${bid}`;if(grouped??groupByCompany)url+=`&group_by=company`;api(url).then(r=>r.json()).then(d=>{if(append){setLeads(p=>[...p,...d]);}else{setLeads(d);}}).catch(()=>{});};
  ...
  // ─── MAIN VIEW ─────────────────────────────────────────
  return(
    <div style={{height:'100vh',display:'flex',flexDirection:'column',background:theme==='dark'?'#020617':'#f8fafc',color:theme==='dark'?'#e2e8f0':'#1e293b',fontFamily:"'DM Sans',-apple-system,sans-serif"}}>
      {showLens&&<LensPanel onClose={()=>setShowLens(false)}/>}
      {showICP&&<ICPPanel onClose={()=>setShowICP(false)}/>}
      {showSettings&&<SettingsPanel user={user} onClose={()=>setShowSettings(false)} onLogout={handleLogout}/>}
      <header style={{display:'flex',alignItems:'center',padding:'10px 20px',borderBottom:'1px solid '+(theme==='dark'?'#111827':'#e2e8f0'),background:theme==='dark'?'#060a14':'#ffffff'}}>
        ...
        <nav style={{display:'flex',gap:2,marginLeft:28,background:theme==='dark'?'#0f172a':'#f1f5f9',borderRadius:8,padding:2}}>{(['chat','leads','scout-lab'] as const).map(v=>(<button key={v} onClick={()=>{setView(v);if(v==='leads')fetchLeads(filter);}} style={{padding:'5px 14px',borderRadius:6,border:'none',cursor:'pointer',fontSize:12,fontWeight:500,background:view===v?(theme==='dark'?'#1e293b':'#ffffff'):'transparent',color:view===v?(theme==='dark'?'#f1f5f9':'#1e293b'):(theme==='dark'?'#64748b':'#94a3b8')}}>{v==='scout-lab'?'Scout Lab':v.charAt(0).toUpperCase()+v.slice(1)}</button>))}</nav>
        <div style={{marginLeft:'auto',display:'flex',gap:6}}>
          ...
          <button onClick={()=>{const next=theme==='dark'?'light':'dark';setTheme(next);localStorage.setItem('rgs_theme',next);}} style={{padding:'5px 10px',borderRadius:6,border:'1px solid '+(theme==='dark'?'#fbbf2433':'#1e293b'),background:theme==='dark'?'#fbbf2408':'#f59e0b15',color:theme==='dark'?'#fbbf24':'#92400e',fontSize:13,cursor:'pointer'}} title="Toggle theme">{theme==='dark'?'☀':'☾'}</button>
          <button onClick={()=>setShowSettings(true)} style={{padding:'5px 10px',borderRadius:6,border:'1px solid '+(theme==='dark'?'#1e293b':'#e2e8f0'),background:'transparent',color:theme==='dark'?'#64748b':'#94a3b8',fontSize:14,cursor:'pointer'}} title="Settings">⚙</button>
        </div>
      </header>
      {stats&&<div style={{display:'flex',gap:14,padding:'8px 20px',borderBottom:'1px solid '+(theme==='dark'?'#111827':'#e2e8f0'),fontSize:11,color:theme==='dark'?'#475569':'#94a3b8',alignItems:'center',background:theme==='dark'?'#060a14':'#fafbfc'}}><span style={{color:theme==='dark'?'#e2e8f0':'#1e293b',fontWeight:600}}>{stats.total}</span>{Object.entries(stats.tiers||{}).sort((a,b)=>b[1]-a[1]).map(([tier,n])=>(<span key={tier} style={{display:'flex',alignItems:'center',gap:3}}><span style={{width:6,height:6,borderRadius:3,background:TC[tier]||'#52525b'}}/>{n} {tier}</span>))}</div>}
      {view==='chat'&&(<div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden'}}><div style={{flex:1,overflowY:'auto',padding:'16px 20px 0'}}>{msgs.map((m,i)=>(<div key={i} style={{display:'flex',justifyContent:m.role==='user'?'flex-end':'flex-start',marginBottom:10}}><div style={{maxWidth:'72%',padding:'10px 14px',borderRadius:10,fontSize:13,lineHeight:1.6,whiteSpace:'pre-wrap',wordBreak:'break-word',background:m.role==='user'?'#7c3aed':(theme==='dark'?'#111827':'#f1f5f9'),color:m.role==='user'?'#fff':(theme==='dark'?'#e2e8f0':'#1e293b'),borderBottomRightRadius:m.role==='user'?2:10,borderBottomLeftRadius:m.role==='user'?10:2}}>{m.content}</div></div>))}{busy&&<div style={{display:'flex',marginBottom:10}}><div style={{padding:'10px 14px',borderRadius:10,background:theme==='dark'?'#111827':'#f1f5f9',color:'#64748b',fontSize:13}}>◈ Analyzing...</div></div>}<div ref={endRef}/></div><CsvDropZone onUploaded={m=>{setMsgs(p=>[...p,{role:'assistant',content:m}]);load();}}/><div style={{padding:'10px 20px 16px',borderTop:'1px solid '+(theme==='dark'?'#111827':'#e2e8f0')}}><div style={{display:'flex',gap:6}}><input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>e.key==='Enter'&&!e.shiftKey&&(e.preventDefault(),send())} placeholder="Ask about leads, run analysis, give feedback…" style={{flex:1,padding:'10px 14px',borderRadius:8,border:'1px solid '+(theme==='dark'?'#1e293b':'#e2e8f0'),background:theme==='dark'?'#0a0f1a':'#ffffff',color:theme==='dark'?'#f1f5f9':'#1e293b',fontSize:13,outline:'none'}}/><button onClick={send} disabled={busy} style={{padding:'10px 18px',borderRadius:8,border:'none',background:busy?'#334155':'linear-gradient(135deg,#7c3aed,#ec4899)',color:'#fff',fontWeight:600,cursor:busy?'default':'pointer',fontSize:13}}>Send</button></div></div></div>)}
      {view==='leads'&&(<div style={{flex:1,overflowY:'auto'}}>
        {/* List tabs */}
        {lists.length>1&&<div style={{padding:'8px 14px',display:'flex',gap:4,borderBottom:'1px solid '+(theme==='dark'?'#111827':'#e2e8f0'),overflowX:'auto',flexShrink:0}}>
          <button onClick={()=>{setActiveList(null);setFilter(null);fetchLeads(null,false,null);}} style={{padding:'3px 10px',borderRadius:8,border:'1px solid '+(theme==='dark'?'#1e293b':'#e2e8f0'),background:!activeList?(theme==='dark'?'#1e293b':'#e2e8f0'):'transparent',color:!activeList?(theme==='dark'?'#f1f5f9':'#1e293b'):(theme==='dark'?'#64748b':'#94a3b8'),fontSize:10,cursor:'pointer',fontWeight:600,whiteSpace:'nowrap'}}>All Leads</button>
          {lists.map(l=>(<button key={l.id} onClick={()=>{setActiveList(l.id);setFilter(null);fetchLeads(null,false,l.id);}} style={{padding:'3px 10px',borderRadius:8,border:`1px solid ${activeList===l.id?l.color||'#8b5cf6':(theme==='dark'?'#1e293b':'#e2e8f0')}`,background:activeList===l.id?`${l.color||'#8b5cf6'}15`:'transparent',color:activeList===l.id?l.color||'#8b5cf6':(theme==='dark'?'#64748b':'#94a3b8'),fontSize:10,cursor:'pointer',fontWeight:600,whiteSpace:'nowrap'}}>{l.list_name||l.filename} <span style={{opacity:0.6}}>({l.row_count})</span></button>))}
        </div>}
        {/* Tier filters + group toggle */}
        <div style={{padding:'10px 14px',display:'flex',gap:5,borderBottom:'1px solid '+(theme==='dark'?'#111827':'#e2e8f0'),flexWrap:'wrap',alignItems:'center'}}>
          {['hot','warm','cold','disqualified'].map(t=>(<button key={t} onClick={()=>{const next=filter===t?null:t;setFilter(next);fetchLeads(next,false,activeList);}} style={{padding:'3px 10px',borderRadius:10,border:`1px solid ${TC[t]}44`,background:filter===t?`${TC[t]}20`:'transparent',color:filter===t?TC[t]:(theme==='dark'?'#64748b':'#94a3b8'),fontSize:10,cursor:'pointer',fontWeight:600,textTransform:'uppercase',letterSpacing:0.5}}>{t}</button>))}
          <button onClick={()=>{const next=filter==='scout-lab-source'?null:'scout-lab-source';setFilter(next);fetchLeads(next,false,activeList);}} style={{padding:'3px 10px',borderRadius:10,border:`1px solid ${filter==='scout-lab-source'?'#a78bfa':'#64748b'}44`,background:filter==='scout-lab-source'?'#a78bfa20':'transparent',color:filter==='scout-lab-source'?'#a78bfa':(theme==='dark'?'#64748b':'#94a3b8'),fontSize:10,cursor:'pointer',fontWeight:600,textTransform:'uppercase',letterSpacing:0.5}}>◈ Scout Lab</button>
          <div style={{marginLeft:'auto'}}><button onClick={()=>{const next=!groupByCompany;setGroupByCompany(next);fetchLeads(filter,false,activeList,next);}} style={{padding:'3px 10px',borderRadius:10,border:'1px solid '+(theme==='dark'?'#1e293b':'#e2e8f0'),background:groupByCompany?(theme==='dark'?'#1e293b':'#e2e8f0'):'transparent',color:groupByCompany?(theme==='dark'?'#f1f5f9':'#1e293b'):(theme==='dark'?'#64748b':'#94a3b8'),fontSize:10,cursor:'pointer',fontWeight:600}}>⊞ Group by Company</button></div>
        </div>
        {/* Leads with optional company grouping */}
        {(()=>{if(!groupByCompany)return leads.map(l=><LeadCard key={l.id} lead={l} onFocus={enterFocus}/>);let lastCompany='';return leads.map(l=>{const showHeader=l.company!==lastCompany;lastCompany=l.company;return(<div key={l.id}>{showHeader&&<div style={{padding:'10px 14px 4px',fontSize:12,fontWeight:700,color:theme==='dark'?'#94a3b8':'#475569',borderTop:'1px solid '+(theme==='dark'?'#111827':'#f1f5f9'),marginTop:4,display:'flex',alignItems:'center',gap:6}}><span style={{fontSize:14}}>🏢</span>{l.company||'No Company'}<span style={{fontSize:10,fontWeight:400,color:theme==='dark'?'#475569':'#94a3b8'}}>({leads.filter(x=>x.company===l.company).length})</span></div>}<LeadCard lead={l} onFocus={enterFocus}/></div>);});})()}
        {!leads.length&&<div style={{padding:40,textAlign:'center',color:'#475569',fontSize:13}}>No leads match.</div>}
      ...
      {view==='scout-lab'&&(<div style={{flex:1,overflow:'hidden'}}><ScoutLab theme={theme}/></div>)}
    </div>
  );
}
