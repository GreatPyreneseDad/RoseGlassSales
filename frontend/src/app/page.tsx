...
export default function Home(){
  ...
  const fetchLeads=(t?:string|null,append?:boolean)=>{let url=`/leads?limit=500&offset=${append?leads.length:0}`;if(t==='scout-lab-source'){url+=`&source=scout-lab`;}else if(t){url+=`&tier=${t}`;}api(url).then(r=>r.json()).then(d=>{if(append){setLeads(p=>[...p,...d]);}else{setLeads(d);}}).catch(()=>{});};
